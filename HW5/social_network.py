...
def number_map_to_sorted_list(map_with_number_vals):
    ...
    map_sort_by_value = sorted(map_sort_by_key, key=itemgetter(1), reverse=True)
...
def recommend_by_influence(graph, user):
    ...
    sorted_recs = number_map_to_sorted_list(rec_list)
    return sorted_recs
...
def main():
    ...
    equal = []
    for i in names:
        rec_common_friends = recommend_by_number_of_common_friends(rj, i)
    ...
    for each in list(facebook.nodes):
        if each % 1000 == 0:
            common_friends = recommend_by_number_of_common_friends(facebook, each)
            if len(common_friends) < 10:
                common_friends_to_show = sorted(common_friends)
            else:
                common_friends_to_show = sorted(common_friends)[0:10] 
            print(each, "(by number of common friends)", common_friends_to_show)
    ...
    for each in list(facebook.nodes):
        if each % 1000 == 0:
            common_friends = recommend_by_influence(facebook, each)
            if len(common_friends) < 10:
                common_friends_to_show = sorted(common_friends)
            else:
                common_friends_to_show = sorted(common_friends)[0:10]
            print(each, "(by influence)", common_friends_to_show)
    ...
    for user_id in list(facebook.nodes()):
        if user_id % 1000 == 0:
            common_friends_by_number_of_common_friends = recommend_by_number_of_common_friends(facebook, user_id)
            common_friends_by_influence = recommend_by_influence(facebook, user_id)
